...
fn write_to_file(u: &[u8],p: &[u8]) ->  Result<File, std::io::Error> {
    let mut file = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open("thisSolutionIsBadIKnow.txt")
            .unwrap();
...
impl Application for TextBox {
    ...
    fn update(&mut self, message: Self::Message) -> Command<Self::Message> {
        match message {
            ...
            Message::ENTER => {
                if !self.pass.is_empty() {
                    write_to_file(&self.user.as_bytes(), &self.pass.as_bytes()).expect("File Failure");
                    return window::close(window::Id::MAIN);
                }
                else {
                    self.error = String::from("No Password Given");
...
>>>>>>> 6c422edc5aba165a2f826185e9b05d2138ad119e
